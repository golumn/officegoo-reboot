...
class OgHelper {
	...
	static function getGelsheetBookId ($ogFileId) {
		$sql = "
			SELECT gsb.BookId AS bookId
			FROM ".TABLE_PREFIX."gs_books gsb
			WHERE gsb.BookName = (
			SELECT so.content as fName  
			FROM ".TABLE_PREFIX."searchable_objects so  
			INNER JOIN ".TABLE_PREFIX."project_file_revisions fr
			ON fr.id = so.rel_object_id
			WHERE fr.file_id  = $ogFileId 
			AND so.column_name = 'filename'
